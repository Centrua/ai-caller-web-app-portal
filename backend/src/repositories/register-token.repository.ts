...
export class RegisterTokenRepository {
  public static async create(data: { venue_id: number; token: string }): Promise<RegisterToken> {
    return await RegisterToken.create(data);
  }

  public static async edit(id: number, data: Partial<{ venue_id: number; token: string }>): Promise<RegisterToken | null> {
