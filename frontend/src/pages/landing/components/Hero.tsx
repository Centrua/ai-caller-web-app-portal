...
export default function Hero() {
    return (
        <section className="max-w-6xl mx-auto px-8 pt-12 pb-24 overflow-hidden">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
                {/* Left Side: Text and Buttons with Floating Backdrop Image */}
                <div className="lg:col-span-6 relative">
                    {/* Floating Backdrop Image positioned behind/to the left of the text content */}
                    <motion.div 
                        initial={{ opacity: 0, x: -30, scale: 0.95 }}
                        animate={{ opacity: 1, x: 0, scale: 1 }}
                        transition={{ duration: 0.8, ease: "easeOut" }}
                        className="absolute -top-12 -left-12 w-64 h-72 lg:w-80 lg:h-96 rounded-3xl overflow-hidden shadow-2xl border border-white/40 -z-10 hidden sm:block pointer-events-none opacity-90"
                    >
                        <img
                            src={`${import.meta.env.BASE_URL}register-venue-backdrop.jpg`}
                            alt="Backdrop"
                            onError={(e) => {
                                if (e.currentTarget.src !== window.location.origin + '/register-venue-backdrop.jpg') {
                                    e.currentTarget.src = '/register-venue-backdrop.jpg'
                                }
                            }}
                            className="w-full h-full object-cover"
                        />
                        <div className="absolute inset-0 bg-[#161915]/10 backdrop-brightness-95"></div>
                    </motion.div>
                    ...
                    <motion.div 
                        ...
                        className="text-left space-y-6 relative z-10 sm:pl-4"
                    >
                        <motion.div 
